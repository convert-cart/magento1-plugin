<?php
class Convertcart_Model_Sync extends Mage_Core_Model_Session_Abstract
{
    public function getStoreInfo()
    {
        ...
        $storeInfo['isProductFlatEnabled'] = Mage::Helper('convertcart/sync_cc')->isProductFlatEnabled();
        $storeInfo['isCategoryFlatEnabled'] = Mage::Helper('convertcart/sync_cc')->isProductFlatEnabled();
        $storeInfo['customerConfig'] = Mage::getStoreConfig('customer/account_share/scope');
        $storeInfo['moduleVersion'] = Mage::Helper('convertcart/sync_cc')->getModuleVersion();
        return $storeInfo;
    }
    ...
    {
        $collection = Mage::getModel('convertcart/sync_cc_category')
                    ->getCollection()
                    ->addAttributeToSelect('entity_id');
    ...
    public function getWebsitesData()
    {
        ...
        foreach (Mage::app()->getWebsites() as $website) {
            ...
            foreach ($website->getGroups() as $group) {
                ...
                foreach ($stores as $store) {
                    ...
                    if (is_array($allowedCurrencies) && count($allowedCurrencies) > 1) {
                        ...
                        $storeData['allowed_currencies'] = array();
                    }
            ...
            $websiteData['store_count'] = $storeCount;
    ...
    public function getCurrencyInfo()
    {
        ...
        $currencyData['rate'] = array(); // Initialize rate array
        
        // Check if rates exist for base currency before iterating
        if (isset($rates[$baseCurrencyCode]) && is_array($rates[$baseCurrencyCode])) {
    ...
    {
        $ccModel = Mage::getSingleton('convertcart/sync_cc');
    ...
    public function getProducts($params)
    {
        $ccModel = Mage::getSingleton('convertcart/sync_cc');
        $ccModel->setParams($params);
        if (Mage::Helper('convertcart/sync_cc')->isProductFlatEnabled() and (!$ccModel->productFlatDisabled)) {
            Mage::app()->getStore(1)
    ...
    public function getCategories($params)
    {
        ...
        $categories = Mage::getModel('convertcart/sync_cc_category')
                    ->load($rootid)
                    ->getCollection()
                    ->addAttributeToSelect('*')
    ...
    public function getWishlist($params)
    {
        ...
        foreach ($wishlistCollection as $wishlist) {
            if (!isset($wishlistData[$w])) {
                $wishlistData[$w] = array();
            }
